...
const int   UDP_PORT = 5005;   // receive commands from Unity
const int   ACK_PORT = 5006;   // replies + calibration
const int   IMU_PORT = 5007;   // IMU stream to Unity
...
void sendIMU()
{
  ...
  udpSend.beginPacket(unityIP, IMU_PORT);
  udpSend.print(buf);
  udpSend.endPacket();
}
...
void doCalibrate()
{
  ...
  udpSend.beginPacket(unityIP, ACK_PORT);
  udpSend.print(buf);
  udpSend.endPacket();
...
void handleCommand(String cmd, IPAddress sender)
{
  ...
  if (cmd == "CONNECT") {
    unityIP    = sender;
    unityKnown = true;
    udpSend.beginPacket(sender, ACK_PORT);
    udpSend.print("READY");
    udpSend.endPacket();
